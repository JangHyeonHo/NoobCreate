...
//@WebServlet("/board/*") // /board/articleForm.do 가 BoardController 객체로 들어감.
						// /board/viewArticle.do?articleNO= 글변호
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//글 추가 요청이 들어왔을때 상황/
				...
				//t_board 테이블에 새로운 글을 추가한 후 추가한 새글에 대한 글 번호를 반환받습니다.
				int articleNO = boardService.addArticle(articleVO);
				
				//파일을 첨부한 경우에만 수행합니다.
				if(imageFileName != null && imageFileName.length() !=0){
					
					//temp폴더에 임시로 업로드 된 파일 객체를 생성함.
					File srcFile = new File(ARTICLE_IMAGE_REPO + "\\" + "temp" + "\\" + imageFileName); // 위에 경로가 있으니 이미지폴더 거기 경로 까진들어감.
					
					File destDir = new File(ARTICLE_IMAGE_REPO + "\\" + articleNO);
					//C:\\board\\article_image의 경로 하위에 글 번호로 폴더를 생성 합니다.
					destDir.mkdirs();
					
					//temp 폴더의 파일을 글번호를 이름으로 하는 폴더로 이동 시킵니다. 라는거
			...
			}else if(action.equals("/viewArticle.do")){//글 제목을 클릭했을때 ㄱ상세 글 정보를 검색해줘.
				
				//요청받은값.
				//상세히 볼 글에 대한 글 번호를 request에서 꺼내오기
				String articleNO = request.getParameter("articleNO");
				
				//서비스 역활을 하는 객체 메소드 호출로 하여 검색 요청을 함.
				//반환값 - > 검색한 글 정보가 저장된 Artcion
				articleVO = boardService.viewArticle(Integer.parseInt(articleNO));
			
				//검색한 글 정보가 저장되어 있는 ArticleVO 객체의 정보를 View페이지로 전달 하기위해
				//request내장객체 영역에 저장하여 유지시키기
				request.setAttribute("article",articleVO);
				
				
				//재요청할 뷰페이지 주소를 nextPage변수에저장
