...
public class BoardDAO { //DB작업 비즈니스로직 처리
	...
	// 매개변수로 전달받은 section값과 pageNum을 이용해서 SQL 문으로 조회합니다.
		public List selectAllArticles(Map pagingMap){
			
			List articlesList = new ArrayList();
			
			//전송된 section 값과 pageNum값을 가져옵니다.
			...
			try {
				...
				//section값과 pageNum값으로 레코드 번호의 범위를 조건으로 정합니다.
				//(이름값이 각각 1로 전송되었으면 between 1 and 10이 됩니다.)
				...
				e.printStackTrace();
				System.out.println("selectAllArticles()메소드에서 오류 : " + e);
			}
	...
	public int selectTotArticles(){
		try {
			conn = dataFactory.getConnection();
			String query = "select count(articleNo) from t_board";//전체 글수를 조회함.
			System.out.println(query);
			pstmt = conn.prepareStatement(query);
			ResultSet rs = pstmt.executeQuery();
			if(rs.next()){
			...
			e.printStackTrace();
			System.out.println("selectTotArticles()메소드에서 오류 : " + e);
		}
		return 0; //조회한 글수 = rs.getInt /아니면 0을 반환한다 - 
	}
